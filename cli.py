    ...
    if hasattr(ai_engine, "learning_engine") and ai_engine.learning_engine:
        ...
        if patterns:
            ...
            for i, pattern in enumerate(patterns[:3], 1):
                console.print(
                    f"{i}. {pattern.error_type} → {pattern.fix_strategy} ({pattern.confidence:.1%} confidence)"
                )
...
def autonomous(task, auto_continue, max_iterations, output):
    ...
    console.print("[bold blue]🤖 Starting Autonomous Mode[/bold blue]")
...
def continuous():
    ...
    console.print("[bold blue]🤖 Continuous Autonomous Mode[/bold blue]")
    console.print(
